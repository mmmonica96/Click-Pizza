...
export default function PizzaApp() {
  ...
  const [selectedPizzaForIngredients, setSelectedPizzaForIngredients] = useState(null);
  ...
  const toggleIngredient = (ingredient) => {
    if (extraIngredients.find((i) => i.id === ingredient.id)) {
      setExtraIngredients(extraIngredients.filter((i) => i.id !== ingredient.id));
    } else {
  ...
  const addToCart = (pizza, extras = []) => {
    ...
    const updatedCart = [...currentCart, pizzaWithExtras];
      ...
      <div className="comida-container">
        {pizzas.map((pizza) => (
          <div key={pizza.id} className="pizza-card p-4 flex flex-col items-center">
            <img src={`/img/pizzas/${pizza.img}`} alt={pizza.name} className="imagen-pizza" />
            <span className="pizza-name mb-2 text-center">{pizza.name}</span>
            <span className="precio-texto">{Number(pizza.price).toFixed(2)} €</span>
            ...
            <div className="w-full flex flex-col gap-2 md:flex-row md:justify-between">
              ...
              <button onClick={() => handleAddIngredientsClick(pizza)} className="btn btn-ingredients">
                Añadir ingredientes
              </button>
            </div>
        ...
        {selectedPizzaForIngredients && (
          <div className="modal-ingredients" onClick={() => setSelectedPizzaForIngredients(null)}>
            <div className="modal-ingredients-content" onClick={(e) => e.stopPropagation()}>
              ...
              <button onClick={handleConfirmIngredients} className="btn btn-confirm">
                Añadir al carrito
              </button>
              <button onClick={() => setSelectedPizzaForIngredients(null)} className="btn btn-cancel">
                Cancelar
              </button>
            </div>
          </div>
