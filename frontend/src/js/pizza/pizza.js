    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow">
        <div className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {pizzas.map((pizza) => (
              <div key={pizza.id} className="bg-amber-50 rounded-lg overflow-hidden shadow-md">
                <div className="p-4 flex flex-col items-center">
                  <div className="w-32 h-32 bg-amber-100 rounded-lg mb-4 flex items-center justify-center">
                    <span className="text-2xl font-bold text-amber-800">{pizza.name}</span>
                  </div>
      ...
      <footer className="bg-amber-100 py-4">
        <div className="container mx-auto px-4">
          <p className="text-center text-amber-800">© 2025 Click & Pizza - Todos los derechos reservados</p>
        </div>
      </footer>
    </div>
  );
